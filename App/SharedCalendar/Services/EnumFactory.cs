...
namespace SharedCalendar.Services
{
    public class EnumFactory<TEnum, KInterface> : IEnumFactory<TEnum, KInterface>
    {
        ...
        public const string ServicesNameSpace = "SharedCalendar.Services";
        ...
        public EnumFactory()
        {
            ...
            foreach (TEnum action in Enum.GetValues(typeof(TEnum)))
            {
                try
                {
                    ...
                    var factory = (KInterface)Activator.CreateInstance(Type.GetType(ServicesNameSpace + "." + Enum.GetName(typeof(TEnum), action) + subname));
