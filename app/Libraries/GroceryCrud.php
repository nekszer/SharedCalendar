...
class grocery_CRUD_Model_Driver extends grocery_CRUD_Field_Types
{
    ...
    public function getArrayDifferences(array $one, array $two) { 
        $differences = array();
        foreach ($one as $onekey => $onevalue) {
            foreach ($two as $twokey => $twovalue) {
                if($onekey == $twokey){
                    if($onevalue != $twovalue){
                        $differences["$onekey"] = "[$onevalue => $twovalue]";
                    }
                    break;
                }
            }
    ...
    protected function db_update($state_info)
    {
        ...
        if($validation_result->success)
        {
            ...
            if($this->callback_update === null)
            {
                ...
                // HACK: @nekszer para log
                $primaryKeyName = $this->get_primary_key();
                $row = $this->getRow($primary_key, $primaryKeyName);
                ...
                $differences = $this->getArrayDifferences((array) $row, $update_data, $primary_key, $primaryKeyName, $this->basic_db_table);
                // TODO: Save columns changes in specific changes of colum table
                $this->log($primary_key, $primaryKeyName, (object) $differences, 'updated');
            ...
            else
            {
                ...
                $differences = $this->getArrayDifferences((array) $row, $post_data, $primary_key, $primaryKeyName, $this->basic_db_table);
                // TODO: Save columns changes in specific changes of colum tables
                $this->log($primary_key, $primaryKeyName, (object) $differences, 'updated');
            }
    ...
    protected function db_delete($state_info)
    {
        ...
        if($this->callback_delete === null)
        {
            ...
            // HACK: @nekszer para log
            $primaryKeyName = $this->get_primary_key();
            $row = $this->getRow($primaryKeyValue, $primaryKeyName);
            ...
            $this->log($primaryKeyValue, $primaryKeyName, $row, 'delete');
    ...
    /**
     * Devuelve una fila segun el id
     */
    public function getRow($primaryKeyValue, $primaryKeyName) {
        $currentModel = new \App\Models\GroceryCrudModel();
        $currentModel->setBuilder($this->basic_db_table);
        $currentModel->where($primaryKeyName, $primaryKeyValue);
        $beforeInsertRow = $currentModel->get_row();
        return $beforeInsertRow;
    }

    /**
     * Realiza la accion de log
     */
    public function log($primaryKeyValue, $primaryKeyName, $data, $action) {
        $idUser = "idUsuario";
        ...
        $logModel = new \App\Models\GroceryCrudModel();
        $logModel->setBuilder('log');
        $logModel->setAllowedFields(["json", "action", "id", "primarykey", "table", "idUser"]);
        $logModel->insert($jsonRowData);
    }
...
class grocery_CRUD_Layout extends grocery_CRUD_Model_Driver
{
    ...
    public function setToolbarButtons($toolbarButtons = array()) {
    ...
    protected function showAddForm()
    {
        ...
        $sizeOfColumns = $crudSetup->getColumnsSize($this->basic_db_table);
        $data->sizeOfColumns = $sizeOfColumns;
    ...
    const GroceryCrudCustomMessage = "groceryCrudCustomMessage";
    ...
    protected function delete_layout($delete_result = true)
    {
        ...
        if($delete_result === false)
        {
            ...
            if(session()->getFlashdata(GroceryCrud::GroceryCrudCustomMessage)){
                $error_message = session()->getFlashdata(GroceryCrud::GroceryCrudCustomMessage);
            }
        ...
        else
        {
            ...
            if(session()->getFlashdata(GroceryCrud::GroceryCrudCustomMessage)){
                $success_message = session()->getFlashdata(GroceryCrud::GroceryCrudCustomMessage);
            }
    ...
    protected function insert_layout($insert_result = false)
    {
        ...
        if($insert_result === false)
        {
            // TODO: Poner lenguaje $l->translate('errorInsert');
            $error_message = "No podemos guardar el registro";
            if(session()->getFlashdata(GroceryCrud::GroceryCrudCustomMessage)){
                $error_message = session()->getFlashdata(GroceryCrud::GroceryCrudCustomMessage);
            }
            echo json_encode(array('success' => $insert_result ,'error_message' => $error_message));
        }
        else
        {
    ...
    protected function update_layout($update_result = false, $state_info = null)
    {
        ...
        {
            // TODO: Poner lenguaje $l->translate('errorUpdate');
            $error_message = "No se puede actualizar el registro";
            if(session()->getFlashdata(GroceryCrud::GroceryCrudCustomMessage)){
                $error_message = session()->getFlashdata(GroceryCrud::GroceryCrudCustomMessage);
            }
            echo json_encode(array('success' => $update_result ,'error_message' => $error_message));
    ...
    {
        $this->set_js_lib($this->default_javascript_path.'/jquery_plugins/jquery.string.min.js');
        $this->set_js_config($this->default_javascript_path.'/jquery_plugins/config/jquery.string.config.js');
        ...
        $input = "<input id='field-{$field_info->name}' class='form-control input-maxlenght' name='{$field_info->name}' type='text' value=\"$value\" $extra_attributes />";
        return $input;
    }
    ...
    protected function get_password_input($field_info,$value)
    {
        ...
        if(!empty($field_info->db_max_length))
            $extra_attributes .= "maxlength='{$field_info->db_max_length}'";
        $input = "<input id='field-{$field_info->name}' class='form-control input-maxlenght' name='{$field_info->name}' type='password' value='$value' $extra_attributes />";
        return $input;
    }
